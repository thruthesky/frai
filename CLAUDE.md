...
### 🛑🛑 사람에게 **명령어를 보여주는 것 자체가 금지**다 (가장 강한 규칙)

**"이 명령을 실행해 보세요" 라고 명령어를 제시하는 순간 그 작업은 실패다.** 예외는 없다.

```
❌ "./script/run-frai.sh 로 확인해 주세요"
❌ "npm run smoke:main 을 돌려 보세요"
❌ "npm run dev 로 직접 보시면 됩니다"
❌ "아래 명령으로 확인 부탁드립니다: …"
```

**왜 이렇게까지 강한가 — 2026-08-22 하루에 세 번 같은 사고가 났다:**
1. 사람에게 `./script/run-frai.sh` 를 시켰다 → 앱이 켜지지 않았다(`ERR_INVALID_URL_SCHEME`).
   인공지능은 빌드만 하고 실행해 본 적이 없었다.
2. 고친 뒤 `npm run smoke:main` 을 시켰다 → **엉뚱한 디렉터리에서 실행돼 `ENOENT`**.
   그 명령은 `frai/` 안에서만 도는데 그 사실을 알려주지 않았다.
3. 매번 사람이 오류를 대신 겪고, 그 오류를 복사해 붙여넣어 알려줘야 했다.

**명령어를 보여주는 것은 "당신이 대신 실행하고 결과를 알려 달라" 는 요청이다.**
그것이 곧 사람에게 일을 시키는 것이며, 실행 환경(작업 디렉터리·환경변수·설치 상태)이
사람마다 달라 **새로운 오류를 만들어낸다.** 인공지능은 그 환경을 이미 가지고 있다.

**그러면 어떻게 하는가:**

| 필요한 것 | 인공지능이 할 일 |
|---|---|
| 명령을 실행해야 한다 | **직접 실행한다.** Bash 도구가 있다 |
| 앱이 켜지는지 확인 | `npm run smoke:main` 을 **직접 돌린다** |
| 화면이 어떻게 보이는지 확인 | `npm run capture` 로 **PNG 를 찍어 이미지를 보여준다** |
| 결과 공유 | 명령어가 아니라 **결과**(출력·이미지·판단)를 보여준다 |

사람이 명령어를 알아야 하는 유일한 경우는 **사람이 그 명령을 직접 쓰고 싶다고 물었을 때**다.
그때조차 "확인해 달라" 는 뜻으로 제시하지 않는다.

### 🛑 사람에게 실행·테스트를 시키는 것은 **작업 실패**다 (타협 불가)

**"직접 실행해 보세요" 라고 말하는 순간 그 작업은 완료가 아니라 실패다.** 예외는 없다.
이것은 예의나 편의의 문제가 아니라 **결함이 사용자에게 도달하는 경로**의 문제다.

**2026-08-22 실제 사고 — 이 규칙이 강화된 이유:**
디자인 작업 후 타입 검사(144파일 0에러)·보안 7규칙·유닛 테스트 105개가 **전부 통과**했고,
빌드도 성공했다. 그래서 "검증 완료" 로 보고하고 사람에게 `./script/run-frai.sh` 로 확인해
달라고 했다. **앱은 켜지지도 않았다** — Vite 가 `new URL('.', import.meta.url)` 을 에셋으로
오인해 소스를 `data:` URL 로 인라인했고, 켜는 순간 `ERR_INVALID_URL_SCHEME` 로 죽었다.

**교훈은 "테스트를 더 하자" 가 아니다.**
- **빌드 성공 ≠ 동작.** 번들러는 통과시키고 런타임에서 죽는 결함이 있다.
- **타입·유닛 테스트는 번들을 로드하지 않는다.** 그래서 이 종류를 원리적으로 못 잡는다.
- **자동으로 확인할 수단이 없으면, 그 수단을 만드는 것까지가 작업 범위다.**
  이 사고 뒤 `npm run smoke:main` 을 만들었고, 같은 버그를 1초 만에 잡는 것을 실측했다.

### 금지

- ❌ **"직접 실행해 보세요" · "직접 만져보세요" · "확인해 주세요" · "테스트해 보시고 알려주세요"
     · "잘 되는지 봐 주세요" · "`npm run dev` 로 확인 부탁드립니다"** — 전부 금지
- ❌ **"자동 검증은 여기까지이고 나머지는 사람 몫입니다" 로 검증 부족을 정당화하는 것.**
     "켜지는가 · 동작하는가 · 깨지지 않는가 · **무엇이 그려지는가**" 는 전부 인공지능이 확인한다.
     화면조차 `npm run capture` 로 직접 보고 판단한다(창은 뜨지 않는다). 사람에게 남는 것은
     **취향의 최종 결정**뿐이며, 그것도 인공지능이 **이미지를 보여준 뒤** 묻는 것이지
     "직접 실행해서 보라" 고 시키는 것이 아니다.
- ❌ 앱을 띄워 놓고 사람에게 넘기며 작업을 끝냈다고 보고하는 것
- ❌ 검증하지 않은 것을 "구현 완료" 로 보고하는 것
- ❌ 사람의 확인을 기다리며 다음 작업을 멈추는 것
- ❌ **코드를 고친 뒤 `npm run test:all` 을 돌리지 않고 보고하는 것**

### 보고 전에 반드시 통과해야 하는 것

```bash
npm run test:all    # 타입 → 보안 → 토큰 → 유닛 → 빌드 → **기동 스모크**
```

마지막 `smoke:main` 이 이 사고 뒤에 추가됐다. **Electron 을 실제로 띄워 main 이 끝까지
살아 오르는지 확인한다**(창은 만들지 않고, macOS 에서는 Dock 아이콘도 숨긴다).
빌드 산출물을 실제로 로드하는 유일한 검사이므로 **건너뛰지 말 것.**
...
| 타입 정합성 | `npm run check` (svelte-check + tsc) | 144 파일 | ❌ |
| **기동 스모크 — 빌드본이 실제로 켜지는가** | `npm run smoke:main` | Electron 실제 실행 | ❌ |
| **화면 캡처 — 실제로 무엇이 그려지는가** | `npm run capture` | 라이트·다크 PNG 2장 | ❌ |
| 디자인 토큰 구조(라이트/다크 쌍) | `npm run check:theme` | 6 검사 | ❌ |
